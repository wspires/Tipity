...
//    if (deflateInit2(&stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, (15+16), 8, Z_DEFAULT_STRATEGY) != Z_OK)
    ...
    NSMutableData *compressedData = [NSMutableData dataWithLength:chunkSize];
    do
    {
        if (stream.total_out >= compressedData.length)
        {
            [compressedData increaseLengthBy:chunkSize];
        }
...
- (unsigned int)crc32
{
    ...
    crcval = 0xffffffff;
    for (x = 0, y = max; x < y; x++) {
        crcval = (uInt)(((crcval >> 8) & 0x00ffffff) ^ crc32table[(crcval ^ (*((unsigned char *)bytes + x))) & 0xff]);
    }
