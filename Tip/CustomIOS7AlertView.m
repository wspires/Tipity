...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i = 0; i != buttonTitles.count; ++i)
    {
        ...
        if (ABOVE_IOS7)
        {
            if (i == 0 && buttonTitles.count == 2)
            {
                // If have 2 buttons, then the first button is likely a cancel button and the second button is the default, preferred action, so make the cancel less prominent.
